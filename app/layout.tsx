        ...
        <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="container mx-auto flex h-16 items-center justify-between px-4">
            <Link href="/" className="flex items-center space-x-2 transition-transform hover:scale-105">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" className="h-8 w-8 text-blue-800">
          ...
          <div className="lg:hidden border-t bg-muted/10">
            <div className="flex space-x-8 px-4 py-3 overflow-x-auto no-scrollbar scroll-smooth">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="text-xs font-black uppercase tracking-widest text-muted-foreground whitespace-nowrap hover:text-blue-800 transition-colors">
                  {link.label}
                </Link>
              ))}
            </div>
        ...
        <footer className="border-t bg-card py-10 mt-auto">
          <div className="container mx-auto px-4 text-center space-y-2">
            <p className="text-sm font-medium text-muted-foreground">
              © 2024 <span className="font-bold text-foreground">VersoKit</span> | Built with <span className="text-blue-800 font-bold">Project IDX</span>
            </p>
